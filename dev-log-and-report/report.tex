...
% A4, margins: left 3cm, right 2cm, top 2cm, bottom 2cm
\usepackage[a4paper, left=3cm, right=2cm, top=2cm, bottom=2cm]{geometry}
% Times New Roman
\usepackage{times}
% Single line spacing + 3pt paragraph spacing
\usepackage{setspace}
\singlespacing
...
\begin{document}
	...
	\begin{abstract}
		...
		The main operation for training machine learning algorithms is the GEMM (General Matrix Multiplication). 
		
		During training, each matrix multiplication operation outputs a matrix which is then chained onto other matrix multiplication operations. As operations persist, buffers and intermediates accumulate and persist and accumulate until garbage collection or freed by explicit rules. On systems utilizing garbage collection, buffer lifetimes could accumulate to be a real problem on memory-constrained systems or systems under heavy workload. Allocation/deallocation overhead can compound, which costs performance, and repeated malloc/free cycles cost latency across a long chain.
		
		As such, the primary design aims not to optimize the GEMM kernel, but to optimize the lifetime of intermediate tensor buffers and temporary-buffer allocation by ensuring buffers, pointers, and cache are freed as soon as possible.
	\end{abstract}
	...
	Destructive-read is more commonly practiced in hardware and circuit design than in software. Dybdahl et al.~\cite{DybdahlKGN06} studied destructive-read behavior in embedded DRAM and found that by removing the write-back buffer without using the cache for reading dataand not  immediately restoring it to DRAM, writing it back only when the cache line has been replaced, there was found to be an immense power and performance benefit.  While specific to a hardware memory system, it motivated this project to investigate wether the DRAM on GPU's will exibit the same behaviour while implemented similarly.
